				...
				>
					...
					>
						...
						>
							...
							<template v-slot:title>部门管理</template>
						</el-menu-item>
						<el-menu-item
							index="role"
					...
					>
						...
						>
							<SvgIcon name="warehouse_fill" class="icon-svg" />
							<template v-slot:title>会议室</template>
						</el-menu-item>
						<el-menu-item
							index="demo-echarts"
						...
						>
							...
							<template v-slot:title>线下会议</template>
						</el-menu-item>
						<el-menu-item
							index="demo-echarts"
						...
						>
							...
							<template v-slot:title>线上会议</template>
						</el-menu-item>
					</el-submenu>
					<el-submenu
						index="系统设置"
					...
					>
						...
						>
							...
							<template v-slot:title>罚款类型</template>
						</el-menu-item>
					</el-submenu>
				</el-menu>
			</div>
				...
				>
					<el-dropdown class="site-tabs__tools" trigger="click">
						...
						<template v-slot:dropdown>
							<el-dropdown-menu >
								<el-dropdown-item v-on:click="tabsCloseCurrentHandle">
									关闭当前标签页
								</el-dropdown-item>
								<el-dropdown-item v-on:click="tabsCloseOtherHandle">
									关闭其它标签页
								</el-dropdown-item>
								<el-dropdown-item v-on:click="tabsCloseAllHandle">
									关闭全部标签页
								</el-dropdown-item>
							</el-dropdown-menu>
						</template>
...
export default {
	components: { SvgIcon, UpdatePassword },	//声明自定义标签
	data: function() {
		return {
			navbarLayoutType: '',
			sidebarFold: false,
	...
	/**
	 * 添加了轮询的 ping 请求，防止WebSocket连接因为超时被切断
	 */
	created() {
		let that = this;
	...
	methods: {
		...
		updatePasswordHandle: function(){
			...
			//通过nextTick回调函数，进行init()函数的回调， 其中updatePassord 为自定义主键update-password标签的 res(命名)，可以用来表示该标签，即整条语句表示该标签调用init函数 
		...
		logout: function(){
			...
			that.$http('user/logout', 'GET', null, 'true', function(resp){
				...
				//退出登陆后没必要保存permisssions，所以删除掉permissions
